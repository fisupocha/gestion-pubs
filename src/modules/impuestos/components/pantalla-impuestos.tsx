...
function Bloque({
  ...
  return (
    <section
      className={`rounded-[22px] border border-[#b2b08d] bg-[linear-gradient(180deg,rgba(243,243,235,0.99)_0%,rgba(228,229,218,0.99)_100%)] p-3 shadow-[0_16px_34px_rgba(66,67,44,0.09)] 2xl:rounded-[24px] 2xl:p-4 2xl:shadow-[0_20px_40px_rgba(66,67,44,0.11)] ${className}`}
    >
      <div className="mb-2 text-center text-[10px] font-black uppercase tracking-[0.22em] text-[#767252] 2xl:mb-3 2xl:text-[11px] 2xl:tracking-[0.24em]">
        {titulo}
      </div>
      {children}
    </section>
...
function Campo({
  ...
  return (
    <label className="grid gap-1.5">
      <span className="text-center text-[10px] font-bold uppercase tracking-[0.08em] text-[#76705d] 2xl:text-[11px] 2xl:tracking-[0.1em]">
        {label}
      </span>
      {children}
    </label>
...
export function PantallaImpuestos({
  ...
  useEffect(() => {
    ...
    async function cargarPersistidos() {
      try {
        const persistidos = await listarImpuestosPersistidos(clasificacionActiva);
        ...
        if (!cancelado) {
          window.alert("No se pudieron cargar los impuestos guardados.");
        }
      }
    }
  ...
  const totalIva = 0;
  const totalImpuesto = round2(base21);
  ...
  async function guardarPrueba(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (totalImpuesto === 0) {
      window.alert("El impuesto debe tener importe distinto de cero.");
      return;
    }
    ...
    try {
      ...
      const persistido = (await guardarImpuestoPersistido(
        registroActual,
        clasificacionActiva
      )) as RegistroFactura;
  ...
  function obtenerCamposBusqueda(registro: RegistroFactura) {
    ...
    const totalImpuestoRegistro = round2(parseDecimal(registro.base21));
    ...
    return [
      ...
      fmtMoney(totalImpuestoRegistro),
      "pagado",
  ...
  return (
    <section className="flex h-full min-h-0 flex-col gap-2 p-2.5 2xl:gap-2.5 2xl:p-3">
      <header className="flex items-center justify-between rounded-[22px] border border-[#b2b08d] bg-[linear-gradient(180deg,#f2f2ea_0%,#e4e4d6_100%)] px-3 py-1 shadow-[0_16px_34px_rgba(66,67,44,0.09)] 2xl:rounded-[24px] 2xl:px-4 2xl:py-1.5 2xl:shadow-[0_18px_38px_rgba(66,67,44,0.11)]">
        <div className="w-[48px] shrink-0 2xl:w-[56px]" />

        <div className="flex-1 text-center">
          <h1 className="text-[18px] font-black tracking-tight text-[#343224] 2xl:text-[20px]">
            Impuestos
          </h1>
        </div>

        <div className="rounded-2xl border border-[#b2b08d] bg-[linear-gradient(180deg,#f6f6f0_0%,#e8e8dd_100%)] px-2 py-0.75 text-center shadow-[0_10px_18px_rgba(74,74,49,0.09)] 2xl:px-2.5 2xl:py-1">
          <div className="text-[8px] font-black uppercase tracking-[0.2em] text-[#767252]">
            ID
          </div>
          <div className="text-base font-black text-[#343224] 2xl:text-lg">{idVisible}</div>
        </div>
      </header>
          ...
          <Bloque titulo="Bases e importes">
            ...
            <div className="mt-2 grid gap-2 lg:grid-cols-[1fr_1fr_1.08fr] 2xl:mt-2.5 2xl:gap-2.5 2xl:grid-cols-[1fr_1fr_1.1fr]">
              <div className={tarjetaDeshabilitadaClassName}>
                <div className="text-center text-[11px] font-black uppercase tracking-[0.16em] text-[#8c8975]">
                  Total base
                </div>
                <div className="mt-0.5 text-center text-[18px] font-black text-[#6f6b58] 2xl:text-[22px]">
              ...
              <div className="rounded-2xl border border-[#8c9362] bg-[linear-gradient(180deg,#e5e9d5_0%,#c4cb96_100%)] px-3.5 py-1.5 text-center shadow-[0_14px_26px_rgba(92,100,45,0.16)] 2xl:px-4 2xl:py-2">
                <div className="text-center text-[11px] font-black uppercase tracking-[0.18em] text-[#4f6320]">
                  Total impuesto
                </div>
                <div className="mt-0.5 text-center text-[22px] font-black text-[#394b14] 2xl:text-[26px]">
                  {fmtMoney(totalImpuesto)} EUR
                </div>
              </div>
            </div>

            <div className="mt-4 rounded-[20px] border border-[#b2b08d] bg-[linear-gradient(180deg,rgba(244,244,236,0.96)_0%,rgba(228,229,219,0.99)_100%)] px-3 py-3 shadow-[0_14px_26px_rgba(66,67,44,0.08)] 2xl:mt-5 2xl:px-4 2xl:py-4">
              <div className="flex flex-wrap items-center justify-center gap-2">
                ...
                <div className="min-w-[80px] rounded-2xl border border-[#bfbcaa] bg-[linear-gradient(180deg,#fbfbf8_0%,#e7e7dc_100%)] px-3 py-2 text-center text-sm font-black text-[#3c3a29] shadow-[0_10px_18px_rgba(74,74,49,0.09)] 2xl:min-w-[88px] 2xl:px-4 2xl:py-2.5">
                  ID {idVisible}
                </div>
                  ...
                  className={
                    botonGuardado
                      ...
                      : !formularioValido
                        ...
                        : "min-w-[124px] rounded-2xl border border-[#b6b397] bg-[linear-gradient(180deg,#fafaf6_0%,#e4e4d5_100%)] px-4 py-2 text-[15px] font-semibold text-[#3c3a29] shadow-[0_12px_20px_rgba(74,74,49,0.10)] transition hover:bg-[#f2f2ea] 2xl:min-w-[136px] 2xl:py-2.5"
                  }
                >
        ...
        <div className="relative grid min-h-0 gap-2.5 2xl:gap-3">
          <Bloque titulo="Pago">
            <div className="grid gap-2.5 lg:grid-cols-[1fr_1fr] 2xl:gap-3">
              ...
              <button
                ...
                disabled
                aria-disabled="true"
                className="cursor-not-allowed rounded-2xl border border-[#d3d1c1] bg-[linear-gradient(180deg,#fafaf6_0%,#ecece4_100%)] px-4 py-2 text-sm font-black text-[#8c8975] opacity-70 shadow-none 2xl:py-2.5"
              >
                No pagado
              </button>
            </div>
              ...
              <div className="rounded-2xl border border-[#b2b08d] bg-[linear-gradient(180deg,#f2f2ea_0%,#e1e2d5_100%)] px-3 py-2 shadow-[0_10px_18px_rgba(74,74,49,0.08)]">
                ...
                <div className="mt-2 flex items-center justify-center gap-2">
                  ...
                  <button
                    ...
                    className="rounded-xl border border-[#b6b397] bg-[linear-gradient(180deg,#fafaf6_0%,#e4e4d5_100%)] px-3 py-1.5 text-[11px] font-semibold text-[#3c3a29] shadow-[0_8px_14px_rgba(74,74,49,0.09)] transition hover:bg-[#f2f2ea]"
                  >
                  ...
                  <button
                    ...
                    className="rounded-xl border border-[#c1beab] bg-[linear-gradient(180deg,#fbfbf8_0%,#e8e8de_100%)] px-3 py-1.5 text-[11px] font-semibold text-[#3c3a29] shadow-[0_8px_14px_rgba(74,74,49,0.09)] transition hover:bg-[#f2f2ea] disabled:cursor-not-allowed disabled:opacity-55"
                  >
                    Ver
                  </button>
          ...
          {buscadorAbierto ? (
            <div className="absolute left-1/2 top-[356px] z-40 w-[320px] -translate-x-1/2">
              <div className="rounded-[24px] border border-[#646345] bg-[linear-gradient(180deg,#3b3b28_0%,#2d2d1f_48%,#222218_100%)] p-5 shadow-[0_24px_56px_rgba(33,33,20,0.30)]">
                <div className="text-center text-[10px] font-black uppercase tracking-[0.16em] text-[#d8d8bf]">
                  Buscar
                </div>

                <div className="mt-2 grid gap-3">
                  <label className="grid gap-1.5">
                    <span className="text-[10px] font-black uppercase tracking-[0.14em] text-[#d8d8bf]">
                      Texto
                    </span>
                    <input
                      ...
                      type="text"
                      className="w-full rounded-2xl border border-[#c5c2aa] bg-[linear-gradient(180deg,#fdfdf9_0%,#f1f1e8_100%)] px-3 py-2 text-sm text-[#2d2c1f] shadow-[inset_0_1px_0_rgba(255,255,255,0.86)] outline-none"
                    />
                  </label>

                  <label className="flex items-center justify-center gap-2 text-sm font-medium text-[#ececdf]">
                    <input
                  ...
                  {mensajeBusqueda ? (
                    <div className="rounded-2xl border border-[#838260] bg-[rgba(251,251,245,0.92)] px-3 py-2 text-center text-sm text-[#595842]">
                      {mensajeBusqueda}
                    </div>
                  ) : null}
                  ...
                  <div className="grid gap-2">
                    <button
                      ...
                      className="rounded-2xl border border-[#b6b397] bg-[linear-gradient(180deg,#fafaf6_0%,#e4e4d5_100%)] px-4 py-2 text-sm font-semibold text-[#3c3a29] shadow-[0_10px_18px_rgba(74,74,49,0.09)]"
                    >
                      Buscar
                    </button>
                    ...
                    <button
                      ...
                      className="rounded-2xl border border-[#6a694c] bg-[linear-gradient(180deg,#44442f_0%,#333325_100%)] px-4 py-2 text-sm font-semibold text-[#ececdf] shadow-[0_8px_16px_rgba(33,33,20,0.22)]"
                    >
                      Cerrar
                    </button>
                  </div>
      ...
      {dialogoSalida.abierto ? (
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-[rgba(22,16,12,0.38)] p-4">
          <div className="w-full max-w-md rounded-[24px] border border-[#b2b08d] bg-[linear-gradient(180deg,#f6f6f0_0%,#e4e4d6_100%)] p-5 shadow-[0_24px_56px_rgba(33,33,20,0.28)]">
            <div className="text-lg font-black text-[#211915]">No has guardado el registro</div>
            <p className="mt-2 text-sm text-[#6f6d5a]">
