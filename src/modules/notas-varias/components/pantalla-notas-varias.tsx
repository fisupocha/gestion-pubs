...
export function PantallaNotasVarias({
  ...
  useEffect(() => {
    ...
    async function cargarPersistidos() {
      try {
        const persistidos = await listarNotasVariasPersistidas(clasificacionActiva);
        ...
        if (!cancelado) {
          window.alert("No se pudieron cargar las notas varias guardadas.");
        }
      }
    }
  ...
  async function guardarPrueba(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const persistido = (await guardarNotaVariaPersistida(
        registroActual,
        clasificacionActiva
      )) as RegistroFactura;
