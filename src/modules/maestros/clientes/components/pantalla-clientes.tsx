...
type AccionCliente = (formData: FormData) => void | Promise<void>;

type PantallaClientesProps = {
  clientes: Cliente[];
  mensaje?: string;
  tipo?: string;
  accionCrear: AccionCliente;
  accionActualizar: AccionCliente;
  accionEliminar: AccionCliente;
};
...
const inputClassName =
  "w-full rounded-2xl border border-[#d7bbb3] bg-[linear-gradient(180deg,#fffaf8_0%,#f5ece8_100%)] px-4 py-3 text-center text-sm text-[#2e211d] shadow-[inset_0_1px_0_rgba(255,255,255,0.86),0_6px_14px_rgba(85,52,46,0.05)] outline-none transition placeholder:text-[#a78f88] focus:border-[#c18173] focus:bg-white focus:shadow-[0_0_0_3px_rgba(193,129,115,0.14)]";

const accionClassName =
  "min-w-[122px] rounded-2xl border border-[#cfafa8] bg-[linear-gradient(180deg,#fbf7f6_0%,#e8dbd8_100%)] px-4 py-2.5 text-[15px] font-semibold text-[#492f29] shadow-[0_12px_20px_rgba(85,52,46,0.10)] transition hover:bg-[#f4edeb]";
...
export function PantallaClientes({
  ...
  const clientesFiltrados = useMemo(() => {
    ...
    return clientes.filter((cliente) => {
      ...
      const cifNormalizado = cliente.cif.toLowerCase();
    ...
    <section className="flex h-full min-h-0 flex-col gap-2 p-2.5 2xl:gap-2.5 2xl:p-3">
      <header className="rounded-[24px] border border-[#d8b4aa] bg-[linear-gradient(180deg,#f8efec_0%,#f2e6e2_100%)] px-5 py-4 shadow-[0_18px_40px_rgba(85,52,46,0.10)]">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <h1 className="mt-2 text-3xl font-black text-[#4b312b]">Clientes</h1>
          ...
          <div className="rounded-[22px] border border-[#d1a79d] bg-[linear-gradient(180deg,#fdf9f8_0%,#ede1dd_100%)] px-5 py-3 text-center shadow-[0_10px_18px_rgba(85,52,46,0.08)]">
            ...
            <div className="mt-1 text-2xl font-black text-[#5b3a33]">{clientes.length}</div>
          </div>
        </div>
      </header>
      ...
      <div className="grid min-h-0 flex-1 gap-2.5 xl:grid-cols-[0.9fr_1.15fr] 2xl:gap-3">
        <section className={`${bloqueClassName} flex min-h-0 flex-col`}>
          ...
          <form
            action={clienteSeleccionadoId === null ? accionCrear : accionActualizar}
            className="flex min-h-0 flex-1 flex-col gap-4"
          >
            <input name="id" type="hidden" value={clienteSeleccionadoId ?? ""} />
            ...
            <div className="rounded-[20px] border border-[#d1a79d] bg-[linear-gradient(180deg,rgba(248,241,239,0.96)_0%,rgba(237,224,220,0.99)_100%)] px-4 py-4 shadow-[0_14px_26px_rgba(85,52,46,0.08)]">
              <div className="flex flex-wrap items-center justify-center gap-3">
                ...
                <button
                  ...
                  disabled={clienteSeleccionadoId === null}
                  aria-disabled={clienteSeleccionadoId === null}
                  className={
                  ...
                  {clientesFiltrados.map((cliente) => {
                    ...
                    return (
                      <button
                        ...
                        className={
                          seleccionado
                            ? "grid w-full grid-cols-[1.3fr_0.8fr_auto] items-center gap-3 bg-[linear-gradient(180deg,#f5e3dc_0%,#edd4cb_100%)] px-5 py-4 text-left text-sm text-[#3f2c28]"
                            : "grid w-full grid-cols-[1.3fr_0.8fr_auto] items-center gap-3 px-5 py-4 text-left text-sm text-[#3f2c28] transition hover:bg-[rgba(232,214,206,0.44)]"
                        }
                      >
                        <span className="font-semibold">{cliente.nombre}</span>
                        <span className="font-medium text-[#6e5751]">{cliente.cif}</span>
