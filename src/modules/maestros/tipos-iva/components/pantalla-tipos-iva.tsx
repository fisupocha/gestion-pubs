...
export function PantallaTiposIva({
  tiposIva,
  mensaje,
  tipo,
  accionCrear,
}: PantallaTiposIvaProps) {
  return (
    <section className="mx-auto max-w-5xl px-6 py-10">
      <div className="mb-8">
        <h1 className="text-3xl font-bold">Tipos de IVA</h1>
      ...
      <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
        ...
        <div className="divide-y divide-gray-200">
          {tiposIva.map((tipoIva) => (
            ...
            >
              <span>{tipoIva.porcentaje}%</span>
              <span className="text-sm text-gray-500">ID {tipoIva.id}</span>
            </div>
          ))}
        </div>
      </div>
