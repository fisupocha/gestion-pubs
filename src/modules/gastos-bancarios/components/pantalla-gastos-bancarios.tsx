...
const TIPO_FIJO_GASTO_BANCARIO = "fijos";
const ETIQUETAS_FAMILIA_GASTO_BANCARIO = ["Gastos banco", "Gastos bancarios"] as const;
...
function resolverFamiliaGastoBancario(clasificacion: ClasificacionMapa) {
  const familias = clasificacion[TIPO_FIJO_GASTO_BANCARIO]?.familias ?? {};
  const entrada = Object.entries(familias).find(([, item]) =>
    ETIQUETAS_FAMILIA_GASTO_BANCARIO.some(
      (etiqueta) => normalizarTexto(item.label) === normalizarTexto(etiqueta)
    )
  );

  return {
    id: entrada?.[0] ?? "gastos-banco",
    label: entrada?.[1]?.label ?? "Gastos banco",
  };
}
...
function crearFormularioInicial(formaPago = "", banco = ""): FormularioFactura {
  return {
    ...
    fechaFactura: "",
    numeroFactura: "",
    tipo: TIPO_FIJO_GASTO_BANCARIO,
    familia: "gastos-banco",
    subfamilia: "",
    base0: "",
    base4: "",
    base10: "",
...
const accionClassName =
  "min-w-[54px] rounded-2xl border border-[#adbec2] bg-[linear-gradient(180deg,#f8fbfb_0%,#dfe7e8_100%)] px-3 py-2 text-center text-sm font-semibold text-[#25383c] shadow-[0_10px_18px_rgba(45,63,68,0.10)] transition duration-150 hover:-translate-y-[2px] hover:scale-[1.02] hover:border-[#365a62] hover:bg-[#f5fbfc] hover:shadow-[0_22px_36px_rgba(45,63,68,0.22)] active:translate-y-0 active:scale-100 2xl:min-w-[60px] 2xl:py-2.5";
...
const campoFijoBloqueadoClassName =
  "pointer-events-none cursor-not-allowed appearance-none !border-[#cfdadd] !bg-[linear-gradient(180deg,#f8fbfb_0%,#e8eef0_100%)] !text-[#7f9094] !shadow-none opacity-70 hover:translate-y-0 hover:scale-100 hover:!border-[#cfdadd] hover:!bg-[linear-gradient(180deg,#f8fbfb_0%,#e8eef0_100%)] hover:!shadow-none focus:!border-[#cfdadd] focus:!bg-[linear-gradient(180deg,#f8fbfb_0%,#e8eef0_100%)] focus:!shadow-none";
...
export function PantallaGastosBancarios({
  ...
  const formaPagoFija =
    ...
    opcionesFormaPago[0] ??
    "";
  const familiaGastoBancario = useMemo(
    () => resolverFamiliaGastoBancario(clasificacionActiva),
    [clasificacionActiva]
  );
  const tipoLabelFijo = clasificacionActiva[TIPO_FIJO_GASTO_BANCARIO]?.label ?? "Fijos";
  ...
  const [snapshotInicial, setSnapshotInicial] = useState(() =>
    crearSnapshot(crearFormularioInicial(formaPagoFija), null)
  );
  ...
  async function guardarPrueba(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const registroActual: RegistroFactura = {
        ...
        ...formulario,
        tipo: TIPO_FIJO_GASTO_BANCARIO,
        familia: familiaGastoBancario.id,
        subfamilia: "",
        ...
        <div className="relative grid min-h-0 gap-2.5 2xl:gap-3">
          <Bloque titulo="Datos principales">
            <div className="grid gap-2.5 lg:grid-cols-[126px_minmax(0,1fr)_144px_144px] 2xl:gap-3 2xl:grid-cols-[140px_minmax(0,1fr)_150px_150px]">
              ...
              <Campo label="Numero factura">
                <input
                  ...
                  aria-disabled="true"
                  type="text"
                  className={`${inputClassName} ${campoDeshabilitadoClassName} text-[#667b80]`}
                />
              </Campo>
            </div>
            ...
            <div className="mt-2.5 grid gap-2.5 lg:grid-cols-3 2xl:mt-3 2xl:gap-3">
              <Campo label="Tipo">
                <select
                  value={TIPO_FIJO_GASTO_BANCARIO}
                  disabled
                  aria-disabled="true"
                  className={`${inputClassName} ${campoDeshabilitadoClassName}`}
                >
                  <option value={TIPO_FIJO_GASTO_BANCARIO}>{tipoLabelFijo}</option>
                </select>
              </Campo>

              <Campo label="Familia">
                <select
                  value={familiaGastoBancario.id}
                  disabled
                  aria-disabled="true"
                  className={`${inputClassName} ${campoDeshabilitadoClassName}`}
                >
                  <option value={familiaGastoBancario.id}>{familiaGastoBancario.label}</option>
                </select>
              </Campo>
